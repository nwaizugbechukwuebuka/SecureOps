...
class TestComplianceFrameworks:
    ...
    async def test_get_frameworks_success(self, test_client, auth_headers):
        ...
        with patch('src.api.routes.compliance.get_current_user') as mock_get_user, \
             patch('src.api.routes.compliance.get_db') as mock_get_db, \
             patch('src.api.services.compliance_service.get_available_frameworks') as mock_get_frameworks:
            ...
            response = await test_client.get("/api/v1/compliance/frameworks", 
    ...
    async def test_get_framework_details(self, test_client, auth_headers):
        ...
        with patch('src.api.routes.compliance.get_current_user') as mock_get_user, \
             patch('src.api.routes.compliance.get_db') as mock_get_db, \
             patch('src.api.services.compliance_service.get_framework_details') as mock_get_details:
            ...
            response = await test_client.get("/api/v1/compliance/frameworks/owasp_top_10", 
                                           headers=auth_headers)
...
class TestComplianceAssessment:
    ...
    async def test_get_compliance_overview(self, test_client, sample_compliance_data, auth_headers):
        ...
        with patch('src.api.routes.compliance.get_current_user') as mock_get_user, \
             patch('src.api.routes.compliance.get_db') as mock_get_db, \
             patch('src.api.services.compliance_service.get_compliance_overview') as mock_get_overview:
    ...
    async def test_get_compliance_by_framework(self, test_client, auth_headers):
        ...
        with patch('src.api.routes.compliance.get_current_user') as mock_get_user, \
             patch('src.api.routes.compliance.get_db') as mock_get_db, \
             patch('src.api.services.compliance_service.get_framework_compliance') as mock_get_compliance:
            ...
            response = await test_client.get("/api/v1/compliance", 
                                           params=params,
                                           headers=auth_headers)
    ...
    async def test_run_compliance_assessment(self, test_client, auth_headers):
        ...
        with patch('src.api.routes.compliance.get_current_user') as mock_get_user, \
             patch('src.api.routes.compliance.get_db') as mock_get_db, \
             patch('src.api.services.compliance_service.run_assessment') as mock_run_assessment:
            ...
            response = await test_client.post("/api/v1/compliance/assess", 
...
class TestComplianceControls:
    ...
    async def test_update_control_status(self, test_client, auth_headers):
        ...
        with patch('src.api.routes.compliance.get_current_user') as mock_get_user, \
             patch('src.api.routes.compliance.get_db') as mock_get_db, \
             patch('src.api.services.compliance_service.update_control_status') as mock_update:
            ...
            response = await test_client.patch("/api/v1/compliance/controls/A01", 
                                             json=update_data,
                                             headers=auth_headers)
    ...
    async def test_get_control_details(self, test_client, auth_headers):
        ...
        with patch('src.api.routes.compliance.get_current_user') as mock_get_user, \
             patch('src.api.routes.compliance.get_db') as mock_get_db, \
             patch('src.api.services.compliance_service.get_control_details') as mock_get_control:
            ...
            response = await test_client.get("/api/v1/compliance/controls/A01", 
                                           headers=auth_headers)
...
class TestComplianceReporting:
    ...
    async def test_generate_compliance_report(self, test_client, auth_headers):
        ...
        with patch('src.api.routes.compliance.get_current_user') as mock_get_user, \
             patch('src.api.routes.compliance.get_db') as mock_get_db, \
             patch('src.api.services.compliance_service.generate_report') as mock_generate:
            ...
            response = await test_client.post("/api/v1/compliance/report", 
                                            json=report_data,
                                            headers=auth_headers)
    ...
    async def test_download_compliance_report(self, test_client, auth_headers):
        ...
        with patch('src.api.routes.compliance.get_current_user') as mock_get_user, \
             patch('src.api.routes.compliance.get_db') as mock_get_db, \
             patch('src.api.services.compliance_service.get_report_file') as mock_get_file:
            ...
            response = await test_client.get("/api/v1/compliance/report/report_123", 
                                           headers=auth_headers)
    ...
    async def test_get_compliance_trends(self, test_client, auth_headers):
        ...
        with patch('src.api.routes.compliance.get_current_user') as mock_get_user, \
             patch('src.api.routes.compliance.get_db') as mock_get_db, \
             patch('src.api.services.compliance_service.get_compliance_trends') as mock_get_trends:
            ...
            response = await test_client.get("/api/v1/compliance/trends", 
                                           params=params,
                                           headers=auth_headers)
...
class TestComplianceAutomation:
    ...
    async def test_configure_automated_assessment(self, test_client, auth_headers):
        ...
        with patch('src.api.routes.compliance.get_current_user') as mock_get_user, \
             patch('src.api.routes.compliance.get_db') as mock_get_db, \
             patch('src.api.services.compliance_service.configure_automation') as mock_configure:
            ...
            response = await test_client.post("/api/v1/compliance/automation", 
    ...
    async def test_get_automation_status(self, test_client, auth_headers):
        ...
        with patch('src.api.routes.compliance.get_current_user') as mock_get_user, \
             patch('src.api.routes.compliance.get_db') as mock_get_db, \
             patch('src.api.services.compliance_service.get_automation_status') as mock_get_status:
            ...
            response = await test_client.get("/api/v1/compliance/automation", 
                                           headers=auth_headers)
...
class TestComplianceIntegrations:
    ...
    async def test_sync_with_external_tools(self, test_client, auth_headers):
        ...
        with patch('src.api.routes.compliance.get_current_user') as mock_get_user, \
             patch('src.api.routes.compliance.get_db') as mock_get_db, \
             patch('src.api.services.compliance_service.sync_external_tools') as mock_sync:
            ...
            response = await test_client.post("/api/v1/compliance/sync", 
                                            json=sync_data,
                                            headers=auth_headers)
    ...
    async def test_export_compliance_data(self, test_client, auth_headers):
        ...
        with patch('src.api.routes.compliance.get_current_user') as mock_get_user, \
             patch('src.api.routes.compliance.get_db') as mock_get_db, \
             patch('src.api.services.compliance_service.export_compliance_data') as mock_export:
...
class TestComplianceValidation:
    ...
    async def test_validate_framework_config(self, test_client, auth_headers):
        ...
        with patch('src.api.routes.compliance.get_current_user') as mock_get_user, \
             patch('src.api.services.compliance_service.validate_framework_config') as mock_validate:
            ...
            response = await test_client.post("/api/v1/compliance/validate", 
                                            json=config_data,
                                            headers=auth_headers)
    ...
    async def test_validate_control_mapping(self, test_client, auth_headers):
        ...
        with patch('src.api.routes.compliance.get_current_user') as mock_get_user, \
             patch('src.api.services.compliance_service.validate_control_mapping') as mock_validate:
            ...
            mock_validate.return_value = {
                "valid": False,
                "errors": [
                    "Control A99 does not exist in OWASP Top 10"
                ],
                "suggestions": [
            ...
            response = await test_client.post("/api/v1/compliance/validate-mapping", 
                                            json=mapping_data,
                                            headers=auth_headers)
