...
class TestPipelineCreation:
    ...
    @pytest.mark.asyncio
    async def test_create_pipeline_success(self, test_client, sample_pipeline_data, auth_headers):
        """Test successful pipeline creation"""
        with patch('src.api.routes.pipelines.get_current_user') as mock_get_user, \
             patch('src.api.routes.pipelines.get_db') as mock_get_db, \
             patch('src.api.routes.pipelines.create_pipeline') as mock_create_pipeline, \
             patch('src.api.services.pipeline_services.setup_webhook') as mock_setup_webhook:
    ...
    @pytest.mark.asyncio
    async def test_create_pipeline_invalid_platform(self, test_client, sample_pipeline_data, auth_headers):
    ...
    @pytest.mark.asyncio
    async def test_create_pipeline_missing_repository(self, test_client, sample_pipeline_data, auth_headers):
    ...
    @pytest.mark.asyncio
    async def test_create_pipeline_webhook_setup_failure(self, test_client, sample_pipeline_data, auth_headers):
...
class TestPipelineRetrieval:
    ...
    @pytest.mark.asyncio
    async def test_get_pipeline_by_id_success(self, test_client, mock_pipeline, auth_headers):
        """Test successful pipeline retrieval by ID"""
        with patch('src.api.routes.pipelines.get_current_user') as mock_get_user, \
             patch('src.api.routes.pipelines.get_db') as mock_get_db, \
             patch('src.api.routes.pipelines.get_pipeline_by_id') as mock_get_pipeline:
            ...
            response = await test_client.get("/api/v1/pipelines/1", headers=auth_headers)
    ...
    async def test_get_pipeline_by_id_not_found(self, test_client, auth_headers):
        ...
        with patch('src.api.routes.pipelines.get_current_user') as mock_get_user, \
            ...
            response = await test_client.get("/api/v1/pipelines/999", headers=auth_headers)
...
class TestPipelineUpdate:
    ...
    @pytest.mark.asyncio
    async def test_update_pipeline_success(self, test_client, mock_pipeline, auth_headers):
        """Test successful pipeline update"""
        with patch('src.api.routes.pipelines.get_current_user') as mock_get_user, \
             patch('src.api.routes.pipelines.get_db') as mock_get_db, \
             patch('src.api.routes.pipelines.get_pipeline_by_id') as mock_get_pipeline, \
             patch('src.api.routes.pipelines.update_pipeline') as mock_update_pipeline:
            ...
            response = await test_client.patch("/api/v1/pipelines/1", 
                                             json=update_data, 
                                             headers=auth_headers)
    ...
    @pytest.mark.asyncio
    async def test_pause_pipeline_success(self, test_client, mock_pipeline, auth_headers):
        """Test successful pipeline pause"""
        with patch('src.api.routes.pipelines.get_current_user') as mock_get_user, \
             patch('src.api.routes.pipelines.get_db') as mock_get_db, \
             patch('src.api.routes.pipelines.get_pipeline_by_id') as mock_get_pipeline, \
             patch('src.api.routes.pipelines.pause_pipeline') as mock_pause:
            ...
            response = await test_client.post("/api/v1/pipelines/1/pause", 
                                            headers=auth_headers)
    ...
    @pytest.mark.asyncio
    async def test_resume_pipeline_success(self, test_client, mock_pipeline, auth_headers):
        """Test successful pipeline resume"""
        with patch('src.api.routes.pipelines.get_current_user') as mock_get_user, \
             patch('src.api.routes.pipelines.get_db') as mock_get_db, \
             patch('src.api.routes.pipelines.get_pipeline_by_id') as mock_get_pipeline, \
             patch('src.api.routes.pipelines.resume_pipeline') as mock_resume:
            ...
            response = await test_client.post("/api/v1/pipelines/1/resume", 
                                            headers=auth_headers)
...
class TestPipelineExecution:
    ...
    @pytest.mark.asyncio
    async def test_trigger_pipeline_success(self, test_client, mock_pipeline, auth_headers):
        """Test successful pipeline trigger"""
        with patch('src.api.routes.pipelines.get_current_user') as mock_get_user, \
             patch('src.api.routes.pipelines.get_db') as mock_get_db, \
             patch('src.api.routes.pipelines.get_pipeline_by_id') as mock_get_pipeline, \
             patch('src.api.services.pipeline_services.trigger_pipeline') as mock_trigger:
    ...
    @pytest.mark.asyncio
    async def test_trigger_pipeline_already_running(self, test_client, mock_pipeline, auth_headers):
    ...
    @pytest.mark.asyncio
    async def test_get_pipeline_logs_success(self, test_client, mock_pipeline, auth_headers):
        """Test successful pipeline logs retrieval"""
        with patch('src.api.routes.pipelines.get_current_user') as mock_get_user, \
             patch('src.api.routes.pipelines.get_db') as mock_get_db, \
             patch('src.api.routes.pipelines.get_pipeline_by_id') as mock_get_pipeline, \
             patch('src.api.services.pipeline_services.get_pipeline_logs') as mock_get_logs:
            ...
            mock_logs = {
                "execution_id": str(uuid4()),
                "logs": [
                    {"timestamp": "2023-01-01T10:00:00Z", "level": "INFO", "message": "Pipeline started"},
                    {"timestamp": "2023-01-01T10:01:00Z", "level": "INFO", "message": "Running security scan"},
                    {"timestamp": "2023-01-01T10:02:00Z", "level": "WARN", "message": "Vulnerability found"}
                ],
                "total_lines": 3
            }
            ...
            response = await test_client.get("/api/v1/pipelines/1/logs", 
                                           headers=auth_headers)
...
class TestPipelineDeletion:
    ...
    @pytest.mark.asyncio
    async def test_delete_pipeline_success(self, test_client, mock_pipeline, auth_headers):
        """Test successful pipeline deletion"""
        with patch('src.api.routes.pipelines.get_current_user') as mock_get_user, \
             patch('src.api.routes.pipelines.get_db') as mock_get_db, \
             patch('src.api.routes.pipelines.get_pipeline_by_id') as mock_get_pipeline, \
             patch('src.api.routes.pipelines.delete_pipeline') as mock_delete, \
             patch('src.api.services.pipeline_services.cleanup_webhook') as mock_cleanup:
            ...
            response = await test_client.delete("/api/v1/pipelines/1", headers=auth_headers)
            
            assert response.status_code == status.HTTP_204_NO_CONTENT

    @pytest.mark.asyncio
    async def test_delete_pipeline_with_active_runs(self, test_client, mock_pipeline, auth_headers):
        """Test deletion of pipeline with active runs"""
        with patch('src.api.routes.pipelines.get_current_user') as mock_get_user, \
             patch('src.api.routes.pipelines.get_db') as mock_get_db, \
             patch('src.api.routes.pipelines.get_pipeline_by_id') as mock_get_pipeline, \
             patch('src.api.services.pipeline_services.has_active_runs') as mock_has_runs:
...
class TestPipelineStatistics:
    ...
    async def test_get_pipeline_stats_success(self, test_client, auth_headers):
        ...
        with patch('src.api.routes.pipelines.get_current_user') as mock_get_user, \
             patch('src.api.routes.pipelines.get_db') as mock_get_db, \
             patch('src.api.routes.pipelines.get_pipeline_statistics') as mock_get_stats:
            ...
            response = await test_client.get("/api/v1/pipelines/stats", headers=auth_headers)
...
class TestPipelineWebhooks:
    ...
    async def test_handle_webhook_github_success(self, test_client):
        ...
        with patch('src.api.routes.pipelines.get_db') as mock_get_db, \
             patch('src.api.routes.pipelines.verify_webhook_signature') as mock_verify, \
             patch('src.api.routes.pipelines.get_pipeline_by_webhook_url') as mock_get_pipeline, \
             patch('src.api.services.pipeline_services.process_webhook') as mock_process:
            ...
            response = await test_client.post("/api/v1/pipelines/webhook/github", 
...
class TestPipelineIntegrations:
    ...
    async def test_sync_github_pipelines(self, test_client, auth_headers):
        ...
        with patch('src.api.routes.pipelines.get_current_user') as mock_get_user, \
             patch('src.api.routes.pipelines.get_db') as mock_get_db, \
             patch('src.api.integrations.github_actions.sync_repositories') as mock_sync:
            ...
            response = await test_client.post("/api/v1/pipelines/sync/github", 
                                            headers=auth_headers)
    ...
    async def test_test_integration_connection(self, test_client, auth_headers):
        ...
        with patch('src.api.routes.pipelines.get_current_user') as mock_get_user, \
             patch('src.api.routes.pipelines.test_platform_connection') as mock_test:
            ...
            response = await test_client.post("/api/v1/pipelines/test-connection", 
                                            json=test_data,
                                            headers=auth_headers)
...
class TestPipelineSecurity:
    ...
    @pytest.mark.asyncio
    async def test_get_pipeline_security_scan(self, test_client, mock_pipeline, auth_headers):
        """Test pipeline security scan results retrieval"""
        with patch('src.api.routes.pipelines.get_current_user') as mock_get_user, \
             patch('src.api.routes.pipelines.get_db') as mock_get_db, \
             patch('src.api.routes.pipelines.get_pipeline_by_id') as mock_get_pipeline, \
             patch('src.api.services.pipeline_services.get_security_scan_results') as mock_get_scan:
            ...
            response = await test_client.get("/api/v1/pipelines/1/security-scan", 
                                           headers=auth_headers)
    ...
    @pytest.mark.asyncio
    async def test_trigger_security_scan(self, test_client, mock_pipeline, auth_headers):
        """Test triggering security scan for pipeline"""
        with patch('src.api.routes.pipelines.get_current_user') as mock_get_user, \
             patch('src.api.routes.pipelines.get_db') as mock_get_db, \
             patch('src.api.routes.pipelines.get_pipeline_by_id') as mock_get_pipeline, \
             patch('src.api.services.pipeline_services.trigger_security_scan') as mock_trigger_scan:
            ...
            response = await test_client.post("/api/v1/pipelines/1/security-scan", 
                                            headers=auth_headers)
