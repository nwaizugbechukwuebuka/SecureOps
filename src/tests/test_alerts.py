...
class TestAlertCreation:
    ...
    async def test_create_alert_success(self, test_client, sample_alert_data, auth_headers):
        ...
        with patch('src.api.routes.alerts.get_current_user') as mock_get_user, \
             patch('src.api.routes.alerts.get_db') as mock_get_db, \
             patch('src.api.routes.alerts.create_alert') as mock_create_alert:
            ...
            response = await test_client.post("/api/v1/alerts", 
...
class TestAlertRetrieval:
    ...
    async def test_get_alerts_success(self, test_client, auth_headers):
        ...
        with patch('src.api.routes.alerts.get_current_user') as mock_get_user, \
             patch('src.api.routes.alerts.get_db') as mock_get_db, \
             patch('src.api.routes.alerts.get_alerts') as mock_get_alerts:
    ...
    async def test_get_alert_by_id_success(self, test_client, mock_alert, auth_headers):
        ...
        with patch('src.api.routes.alerts.get_current_user') as mock_get_user, \
             patch('src.api.routes.alerts.get_db') as mock_get_db, \
             patch('src.api.routes.alerts.get_alert_by_id') as mock_get_alert:
...
class TestAlertUpdate:
    ...
    async def test_update_alert_success(self, test_client, mock_alert, auth_headers):
        ...
        with patch('src.api.routes.alerts.get_current_user') as mock_get_user, \
             patch('src.api.routes.alerts.get_db') as mock_get_db, \
             patch('src.api.routes.alerts.get_alert_by_id') as mock_get_alert, \
             patch('src.api.routes.alerts.update_alert') as mock_update_alert:
            ...
            response = await test_client.patch("/api/v1/alerts/1", 
                                             json=update_data, 
                                             headers=auth_headers)
    ...
    async def test_acknowledge_alert_success(self, test_client, mock_alert, auth_headers):
        ...
        with patch('src.api.routes.alerts.get_current_user') as mock_get_user, \
             patch('src.api.routes.alerts.get_db') as mock_get_db, \
             patch('src.api.routes.alerts.get_alert_by_id') as mock_get_alert, \
             patch('src.api.routes.alerts.acknowledge_alert') as mock_acknowledge:
            ...
            response = await test_client.post("/api/v1/alerts/1/acknowledge", 
                                            headers=auth_headers)
    ...
    async def test_resolve_alert_success(self, test_client, mock_alert, auth_headers):
        ...
        with patch('src.api.routes.alerts.get_current_user') as mock_get_user, \
             patch('src.api.routes.alerts.get_db') as mock_get_db, \
             patch('src.api.routes.alerts.get_alert_by_id') as mock_get_alert, \
             patch('src.api.routes.alerts.resolve_alert') as mock_resolve:
            ...
            response = await test_client.post("/api/v1/alerts/1/resolve", 
...
class TestAlertDeletion:
    ...
    async def test_delete_alert_success(self, test_client, mock_alert, auth_headers):
        ...
        with patch('src.api.routes.alerts.get_current_user') as mock_get_user, \
             patch('src.api.routes.alerts.get_db') as mock_get_db, \
             patch('src.api.routes.alerts.get_alert_by_id') as mock_get_alert, \
             patch('src.api.routes.alerts.delete_alert') as mock_delete:
...
class TestAlertBulkOperations:
    ...
    async def test_bulk_acknowledge_success(self, test_client, auth_headers):
        ...
        with patch('src.api.routes.alerts.get_current_user') as mock_get_user, \
             patch('src.api.routes.alerts.get_db') as mock_get_db, \
             patch('src.api.routes.alerts.bulk_acknowledge_alerts') as mock_bulk_ack:
            ...
            response = await test_client.post("/api/v1/alerts/bulk-action", 
                                            json=bulk_data, 
                                            headers=auth_headers)
    ...
    async def test_bulk_resolve_success(self, test_client, auth_headers):
        ...
        with patch('src.api.routes.alerts.get_current_user') as mock_get_user, \
             patch('src.api.routes.alerts.get_db') as mock_get_db, \
             patch('src.api.routes.alerts.bulk_resolve_alerts') as mock_bulk_resolve:
    ...
    async def test_bulk_delete_success(self, test_client, auth_headers):
        ...
        with patch('src.api.routes.alerts.get_current_user') as mock_get_user, \
             patch('src.api.routes.alerts.get_db') as mock_get_db, \
             patch('src.api.routes.alerts.bulk_delete_alerts') as mock_bulk_delete:
...
class TestAlertStatistics:
    ...
    async def test_get_alert_stats_success(self, test_client, auth_headers):
        ...
        with patch('src.api.routes.alerts.get_current_user') as mock_get_user, \
             patch('src.api.routes.alerts.get_db') as mock_get_db, \
             patch('src.api.routes.alerts.get_alert_statistics') as mock_get_stats:
    ...
    async def test_get_alert_trends_success(self, test_client, auth_headers):
        ...
        with patch('src.api.routes.alerts.get_current_user') as mock_get_user, \
             patch('src.api.routes.alerts.get_db') as mock_get_db, \
             patch('src.api.routes.alerts.get_alert_trends') as mock_get_trends:
            ...
            mock_trends = {
                "period": "30d",
                "data": [
                    {"date": "2023-01-01", "count": 5, "severity_breakdown": {"high": 2, "medium": 3}},
                    {"date": "2023-01-02", "count": 8, "severity_breakdown": {"high": 3, "medium": 4, "low": 1}},
                ]
            }
            ...
            response = await test_client.get("/api/v1/alerts/trends", 
                                           params=params, 
                                           headers=auth_headers)
...
class TestAlertNotifications:
    ...
    async def test_alert_webhook_trigger(self, test_client, sample_alert_data, auth_headers):
        ...
        with patch('src.api.routes.alerts.get_current_user') as mock_get_user, \
             patch('src.api.routes.alerts.get_db') as mock_get_db, \
             patch('src.api.routes.alerts.create_alert') as mock_create_alert, \
             patch('src.api.services.alert_service.trigger_alert_webhook') as mock_webhook:
    ...
    async def test_alert_email_notification(self, test_client, sample_alert_data, auth_headers):
        ...
        with patch('src.api.routes.alerts.get_current_user') as mock_get_user, \
             patch('src.api.routes.alerts.get_db') as mock_get_db, \
             patch('src.api.routes.alerts.create_alert') as mock_create_alert, \
             patch('src.api.services.alert_service.send_alert_email') as mock_email:
...
class TestAlertFiltering:
    ...
    async def test_search_alerts_by_text(self, test_client, auth_headers):
        ...
        with patch('src.api.routes.alerts.get_current_user') as mock_get_user, \
             patch('src.api.routes.alerts.get_db') as mock_get_db, \
             patch('src.api.routes.alerts.search_alerts') as mock_search:
            ...
            response = await test_client.get("/api/v1/alerts/search", 
                                           params=params, 
                                           headers=auth_headers)
