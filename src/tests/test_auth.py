...
class TestUserRegistration:
    ...
    async def test_register_user_success(self, test_client, sample_user_data):
        ...
        with patch('src.api.routes.auth.get_db') as mock_get_db, \
             patch('src.api.routes.auth.create_user') as mock_create_user, \
             patch('src.api.routes.auth.send_verification_email') as mock_send_email:
    ...
    async def test_register_user_duplicate_email(self, test_client, sample_user_data):
        ...
        with patch('src.api.routes.auth.get_db') as mock_get_db, \
             patch('src.api.routes.auth.get_user_by_email') as mock_get_user:
...
class TestUserLogin:
    ...
    async def test_login_success(self, test_client, sample_user_data, mock_user):
        ...
        with patch('src.api.routes.auth.authenticate_user') as mock_auth, \
             patch('src.api.routes.auth.create_access_token') as mock_create_token, \
             patch('src.api.routes.auth.create_refresh_token') as mock_create_refresh, \
             patch('src.api.routes.auth.update_last_login') as mock_update_login:
...
class TestTokenManagement:
    ...
    async def test_logout_success(self, test_client, valid_token):
        ...
        with patch('src.api.routes.auth.get_current_user') as mock_get_user, \
             patch('src.api.routes.auth.invalidate_token') as mock_invalidate:
...
class TestPasswordManagement:
    ...
    async def test_change_password_success(self, test_client, valid_token, mock_user):
        ...
        with patch('src.api.routes.auth.get_current_user') as mock_get_user, \
             ...
             patch('src.api.routes.auth.update_password') as mock_update:
    ...
    async def test_reset_password_request(self, test_client):
        ...
        with patch('src.api.routes.auth.get_user_by_email') as mock_get_user, \
             ...
             patch('src.api.routes.auth.send_reset_email') as mock_send_email:
    ...
    async def test_reset_password_confirm(self, test_client):
        ...
        with patch('src.api.routes.auth.verify_reset_token') as mock_verify, \
             patch('src.api.routes.auth.get_user_by_email') as mock_get_user, \
             patch('src.api.routes.auth.update_password') as mock_update:
            ...
            response = await test_client.post("/api/v1/auth/reset-password/confirm", 
                                            json=confirm_data)
...
class TestEmailVerification:
    ...
    async def test_verify_email_success(self, test_client):
        ...
        with patch('src.api.routes.auth.verify_email_token') as mock_verify, \
             patch('src.api.routes.auth.get_user_by_email') as mock_get_user, \
             patch('src.api.routes.auth.activate_user') as mock_activate:
    ...
    async def test_resend_verification_email(self, test_client):
        ...
        with patch('src.api.routes.auth.get_user_by_email') as mock_get_user, \
            ...
            response = await test_client.post("/api/v1/auth/resend-verification", 
                                            json=resend_data)
...
class TestRateLimiting:
    ...
    async def test_registration_rate_limiting(self, test_client, sample_user_data):
        ...
        assert any(status_code in [status.HTTP_201_CREATED, status.HTTP_429_TOO_MANY_REQUESTS] 
