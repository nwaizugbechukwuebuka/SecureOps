...
async def get_vulnerability_summary(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to generate vulnerability summary"
        )
...
async def get_pipeline_performance(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to generate pipeline performance report"
        )


@router.get("/compliance/status")
async def get_compliance_status(
    framework: Optional[str] = Query(None, description="Compliance framework (OWASP, NIST, SOC2, GDPR)"),
    pipeline_id: Optional[int] = Query(None, description="Filter by pipeline ID"),
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db)
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to generate compliance status report"
        )


@router.get("/security/trends")
async def get_security_trends(
    metric: str = Query("vulnerabilities", description="Metric to analyze (vulnerabilities, scans, alerts)"),
...
async def get_alert_analytics(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to generate alert analytics"
        )
...
async def generate_custom_report(
    ...
    try:
        ...
        logger.info(f"Custom report queued for user {current_user.id}, task_id: {task_id}")
...
async def export_vulnerabilities(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to export vulnerability data"
        )
...
async def get_scanner_metrics(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to generate scanner metrics"
        )
...
async def schedule_report(
    ...
    try:
        ...
        schedule_id = await report_service.create_report_schedule(
            user_id=current_user.id,
            schedule_config=report_schedule
        )
        
        logger.info(f"Report schedule created for user {current_user.id}, schedule_id: {schedule_id}")
