...
class AlertService:
    ...
    async def get_user_alerts(
        ...
        try:
            ...
            query = select(Alert).options(
                ...
                selectinload(Alert.vulnerability)
            ).join(Pipeline).where(Pipeline.owner_id == user_id)
    ...
    async def get_alert_by_id(
        ...
        try:
            query = select(Alert).options(
                ...
                and_(Alert.id == alert_id, Pipeline.owner_id == user_id)
            )
    ...
    async def get_alert_statistics(
        ...
        try:
            ...
            status_counts = {status: count for count, status in status_result.fetchall()}
            ...
            severity_counts = {severity: count for count, severity in severity_result.fetchall()}
            ...
            ).group_by(func.date_trunc('day', Alert.created_at)).order_by('date')
        ...
        try:
            ...
            if notifications.get("email", {}).get("enabled", True):
                channels.append({
                    ...
                    "config": {
                        "email": user.email,
                        "severity_filter": notifications.get("email", {}).get("severity_filter", ["critical", "high"])
                    }
                })
            ...
            if notifications.get("slack", {}).get("enabled", False):
                ...
                channels.append({
                    ...
                    "config": {
                        ...
                        "severity_filter": slack_config.get("severity_filter", ["critical", "high"])
                    }
                })
            ...
            if notifications.get("webhook", {}).get("enabled", False):
                ...
                channels.append({
                    ...
                    "config": {
                        ...
                        "severity_filter": webhook_config.get("severity_filter", ["critical", "high"])
                    }
                })
    ...
    async def test_notification_channel(
        ...
        try:
            ...
            return {
                ...
                "message": "Test notification sent successfully" if success else error_message,
                "timestamp": datetime.now(timezone.utc).isoformat()
            }
    ...
    async def create_vulnerability_alert(
        ...
        try:
            ...
            vuln_query = select(Vulnerability).where(Vulnerability.id == vulnerability_id)
    ...
    async def _process_alert_notifications(self, alert: Alert) -> None:
        ...
        try:
            ...
            pipeline_query = select(Pipeline).options(
                selectinload(Pipeline.owner)
            ...
            if notification_tasks:
                ...
                update_query = update(Alert).where(Alert.id == alert.id).values({
    ...
    def _should_notify_channel(self, alert_severity: str, channel: Dict[str, Any]) -> bool:
        """Check if channel should receive notification based on severity filter."""
        severity_filter = channel.get("config", {}).get("severity_filter", ["critical", "high"])
        return alert_severity in severity_filter
    
    async def _send_alert_notification(self, alert: Alert, channel: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.error(f"Error sending notification to {channel.get('type')}: {str(e)}")
    
    async def _send_email_notification(self, email: str, message: Dict[str, Any]) -> bool:
