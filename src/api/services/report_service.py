...
class ReportService:
    ...
    async def get_dashboard_summary(self, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            scan_query = select(func.count(ScanJob.id)).join(Pipeline).where(
                and_(
                    Pipeline.owner_id == user_id,
                    ScanJob.created_at >= recent_date
                )
            )
            ...
            alert_query = select(
                ...
                Alert.status
            ).join(Pipeline).where(Pipeline.owner_id == user_id).group_by(Alert.status)
        ...
        try:
            ...
            for count, severity, date in trends_result.fetchall():
                ...
                if date_str not in trends_by_day:
                    trends_by_day[date_str] = {"critical": 0, "high": 0, "medium": 0, "low": 0}
                trends_by_day[date_str][severity] = count
            
            # Get vulnerability distribution by scanner type
            scanner_query = select(
                func.count(Vulnerability.id).label('count'),
                Vulnerability.scanner_type
            ).select_from(base_query.subquery()).group_by(Vulnerability.scanner_type)
            ...
            ).group_by(Vulnerability.file_path).order_by(desc('count')).limit(10)
            ...
            remediation_query = select(
                func.count(Vulnerability.id).label('count'),
                Vulnerability.status
            ).select_from(base_query.subquery()).group_by(Vulnerability.status)
    ...
    async def get_scan_performance_metrics(
        ...
        try:
            ...
            ).group_by(func.date_trunc('day', ScanJob.created_at)).order_by('date')
    ...
    async def generate_custom_report(
        ...
        try:
            ...
            start_date = datetime.fromisoformat(
                date_range.get("start", (datetime.now(timezone.utc) - timedelta(days=30)).isoformat())
            )
    ...
    async def export_report_csv(
        ...
        try:
            ...
            if report_type == "vulnerability_summary":
                ...
                writer.writerow([
                    "Pipeline", "Vulnerability ID", "Title", "Severity", 
                    "Status", "Scanner", "File Path", "Created Date"
                ])
                ...
                writer.writerow([
                    "Pipeline", "Scan ID", "Status", "Scanner Types", 
                    "Started At", "Completed At", "Duration", "Vulnerabilities Found"
                ])
    ...
    async def _calculate_owasp_compliance(self, user_id: int) -> Dict[str, Any]:
        ...
        owasp_categories = [
            ...
            "security_misconfig", "xss", "insecure_deserialization", "vulnerable_components", "logging"
        ]
        ...
        for category in owasp_categories:
            ...
            vuln_query = select(func.count(Vulnerability.id)).join(Pipeline).where(
                and_(
                    Pipeline.owner_id == user_id,
                    Vulnerability.status.in_(["open", "acknowledged"]),
                    or_(
