...
def send_notification(
    ...
    try:
        ...
        result = asyncio.run(_send_notification_async(
...
async def _send_notification_async(
    ...
    try:
        if channel_type == 'email':
            ...
            return await _send_webhook_notification(recipient, subject, message, metadata)
        elif channel_type == 'msteams':
            return await _send_msteams_notification(recipient, subject, message, metadata)
        else:
    ...
    try:
        async with async_session() as db:
            ...
            if alert.status in ['resolved', 'acknowledged']:
                logger.info(f"Alert {alert_id} already resolved/acknowledged, skipping escalation")
            ...
            escalation_notifications = await _determine_escalation_notifications(alert, escalation_level)
            ...
            next_escalation_delay = _get_escalation_delay(alert.severity, escalation_level + 1)
            if next_escalation_delay and escalation_level < 3:  # Max 3 escalation levels
                escalate_alert.apply_async(
...
async def _create_alert_digest_async(
    ...
    try:
        ...
        async with async_session() as db:
            ...
            digest_content = await _generate_digest_content(alerts, time_range, pipeline_id)
            ...
            for recipient in digest_recipients:
                try:
                    ...
                    logger.error(f"Failed to send digest to {recipient['address']}: {str(e)}")
        ...
        async with async_session() as db:
            ...
            for channel in channels:
                ...
                if _should_notify_channel(channel, alert):
                    notification_content = await _generate_notification_content(alert, channel)
...
async def _determine_escalation_notifications(
    ...
    try:
        ...
        escalation_channels = await _get_escalation_channels(alert.pipeline_id, escalation_level)
        
        for channel in escalation_channels:
            content = await _generate_escalation_content(alert, escalation_level, channel)
...
def _should_notify_channel(channel: NotificationChannel, alert: Alert) -> bool:
    ...
    try:
        ...
        min_severity = severity_levels.get(channel.configuration.get('min_severity', 'low'), 0)
...
async def _render_email_template(message: str, metadata: Dict[str, Any]) -> str:
    ...
    return template.render(
        ...
        severity_color=severity_colors.get(metadata.get('severity', 'medium'), '#808080'),
