...
@celery_app.task(bind=True, name='secureops.tasks.monitor_tasks.pipeline_health_monitor')
def pipeline_health_monitor(self, pipeline_id: Optional[int] = None):
    ...
    """
    logger.info(f"Starting pipeline health monitoring for pipeline {pipeline_id or 'all'}")
    ...
    try:
        ...
        logger.info(f"Pipeline health monitoring completed - Monitored {results['pipelines_checked']} pipelines")
        return results
...
@celery_app.task(bind=True, name='secureops.tasks.monitor_tasks.scanner_availability_check')
def scanner_availability_check(self):
    ...
    try:
        ...
        logger.info(f"Scanner availability check completed - {scanner_status['available_count']}/{scanner_status['total_count']} scanners available")
...
def repository_monitor(self, pipeline_id: int):
    ...
    try:
        ...
        logger.error(f"Repository monitoring failed for pipeline {pipeline_id}: {str(e)}")
        raise


@celery_app.task(bind=True, name='secureops.tasks.monitor_tasks.performance_metrics_collection')
...
@celery_app.task(bind=True, name='secureops.tasks.monitor_tasks.dependency_freshness_check')
def dependency_freshness_check(self):
    ...
    try:
        ...
        logger.info(f"Dependency freshness check completed - {results['outdated_count']} outdated dependencies found")
        return results
...
@celery_app.task(bind=True, name='secureops.tasks.monitor_tasks.integration_health_check')
def integration_health_check(self):
    ...
    try:
        ...
        logger.info(f"Integration health check completed - {integration_status['healthy_count']}/{integration_status['total_count']} integrations healthy")
...
async def _perform_health_check() -> Dict[str, Any]:
    ...
    try:
        ...
        health_status['components']['external_services'] = await _check_external_services()
    ...
    try:
        ...
        for scanner_type in available_scanners:
            ...
            try:
                ...
                scanner_health['response_time'] = (end_time - start_time).total_seconds()
...
async def _monitor_repository_changes(pipeline_id: int) -> Dict[str, Any]:
    ...
    try:
        async with async_session() as db:
            ...
            new_commits = await _get_commits_since(pipeline.repository_url, last_scan_time)
            
            if new_commits:
                changes['has_changes'] = True
                changes['changes_detected'] = new_commits
                changes['last_commit'] = new_commits[0] if new_commits else None
                changes['files_changed'] = sum(commit.get('files_changed', 0) for commit in new_commits)
...
async def _check_dependency_freshness() -> Dict[str, Any]:
    ...
    try:
        ...
        results['outdated_dependencies'] = [dep for dep in all_deps if dep['is_outdated']]
        results['outdated_count'] = len(results['outdated_dependencies'])
        results['critical_updates'] = len([dep for dep in results['outdated_dependencies'] if dep.get('has_security_update')])
...
async def _check_integration_health() -> Dict[str, Any]:
    ...
    try:
        ...
        integrations_to_check = [
            {'name': 'GitHub API', 'endpoint': 'https://api.github.com/user', 'type': 'github'},
            {'name': 'GitLab API', 'endpoint': 'https://gitlab.com/api/v4/user', 'type': 'gitlab'},
            {'name': 'Azure DevOps', 'endpoint': settings.AZURE_DEVOPS_URL, 'type': 'azure'},
            {'name': 'Jenkins', 'endpoint': settings.JENKINS_URL, 'type': 'jenkins'},
            {'name': 'Slack Webhook', 'endpoint': settings.SLACK_WEBHOOK_URL, 'type': 'slack'},
        ]
...
async def _check_external_services() -> Dict[str, Any]:
    ...
    status = 'healthy' if healthy_count == total_count else 'warning' if healthy_count > 0 else 'critical'
