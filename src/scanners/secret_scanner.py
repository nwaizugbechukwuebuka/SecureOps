...
from .common import (
    ...
    SeverityLevel, FileTypeDetector, orchestrator
)
...
class SecretScanner(BaseScanner):
    ...
    def _initialize_patterns(self) -> Dict[str, List[Pattern]]:
        ...
        patterns = {
            ...
            'connection_strings': [
                ...
                re.compile(r'(?i)data\s+source\s*=\s*[^;]+;\s*initial\s+catalog\s*=\s*[^;]+;\s*user\s+id\s*=\s*[^;]+;\s*password\s*=\s*[^;]+'),
            ],
            'passwords': [
    ...
    def _get_severity_for_secret_type(self, secret_type: str) -> SeverityLevel:
        ...
        high_severity_types = {
            'private_keys', 'aws_credentials', 'database_urls', 
            'connection_strings', 'passwords'
        }
        
        medium_severity_types = {
            'api_keys', 'github_tokens', 'gitlab_tokens', 'jwt_tokens',
            'slack_tokens', 'stripe_keys', 'paypal_keys'
        }
    ...
    def _calculate_pattern_confidence(self, secret_type: str, secret_value: str) -> float:
        ...
        high_confidence_types = {
            'aws_credentials', 'github_tokens', 'gitlab_tokens', 
            'google_api', 'private_keys', 'certificates'
        }
