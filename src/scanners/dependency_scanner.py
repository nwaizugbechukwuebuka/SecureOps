...
class SafetyScanner(BaseScanner):
    ...
    def _determine_severity(self, advisory: str, cve: str) -> SeverityLevel:
        ...
        high_keywords = ['high', 'sql injection', 'xss', 'csrf', 'authentication bypass']
        medium_keywords = ['medium', 'denial of service', 'dos', 'information disclosure']
...
class BanditScanner(BaseScanner):
    ...
    def _find_python_files(self, target: str) -> List[str]:
        ...
        return [
            file_path for file_path in FileTypeDetector.get_scannable_files(target)
    ...
    async def _run_bandit_scan(
        ...
        try:
            ...
            command = [
                ...
                "-r", target,  # Recursive
                "-f", "json",  # JSON format
                "-l", confidence,  # Confidence level
                "-s", severity     # Severity level
            ]
