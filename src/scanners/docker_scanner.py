...
class TrivyScanner(BaseScanner):
    ...
    def _detect_scan_type(self, target: str) -> str:
        ...
        if os.path.isdir(target):
            ...
            dockerfile_patterns = ['Dockerfile', 'Containerfile', 'docker-compose.yml', 'docker-compose.yaml']
    ...
    def _mask_secret_content(self, content: str) -> str:
        ...
        patterns = [
            (r'(["\']?)([a-zA-Z0-9+/=]{20,})(["\']?)', r'\1***MASKED***\3'),  # Base64-like
            (r'(["\']?)([a-fA-F0-9]{32,})(["\']?)', r'\1***MASKED***\3'),      # Hex strings
            (r'(password\s*[:=]\s*["\']?)([^"\'\\s]+)(["\']?)', r'\1***MASKED***\3'),  # Passwords
            (r'(api[_-]?key\s*[:=]\s*["\']?)([^"\'\\s]+)(["\']?)', r'\1***MASKED***\3'),  # API keys
            (r'(token\s*[:=]\s*["\']?)([^"\'\\s]+)(["\']?)', r'\1***MASKED***\3'),  # Tokens
        ]
